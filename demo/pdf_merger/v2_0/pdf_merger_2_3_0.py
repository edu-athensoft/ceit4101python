...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("Athensoft PDF Helper - Merger v2.3")
        ...
        # Create table frame
        self.table_frame = customtkinter.CTkFrame(self.home_frame, corner_radius=0, fg_color="transparent", bg_color="#ffffff")
    ...
    def update_table(self):
        ...
        for index, file in enumerate(self.selected_files):
            ...
            move_up_button = customtkinter.CTkButton(row_frame, text="Up", width=20,
                                                     command=lambda i=index: self.move_up(i))
            move_up_button.grid(row=0, column=1, padx=3, pady=3)

            move_down_button = customtkinter.CTkButton(row_frame, text="Down", width=20,
                                                       command=lambda i=index: self.move_down(i))
            move_down_button.grid(row=0, column=2, padx=3, pady=3)

            # PDF file path label
            file_label = customtkinter.CTkLabel(row_frame, text=file)
            file_label.grid(row=0, column=3, padx=5, pady=5)
