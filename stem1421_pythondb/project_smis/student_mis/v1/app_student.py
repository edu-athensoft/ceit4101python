...
def viewAll():
    ...
    # print("viewAll()", file=open('log.txt', 'a'))
...
def viewByName(student_name):
    ...
    results = studentdb.get_by_studentname(student_name)
    if len(results):
        for row in results:
            print("{:^10}  {:16}   {:28}{:^6}".format(row[0], row[1], row[2], row[3]))
    else:
    ...
    def inputStudentRecord():
        while True:
            ...
            if studentID:
                ...
                studentRecord = studentdb.get_by_id(studentID)
...
def update():
    ...
    if not studentRecord:
        ...
        if update_option.lower() == 'y':
            ...
            studentdb.update(studentRecord)
